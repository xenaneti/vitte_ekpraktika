...
PUBLIC_PAGES = ["home", "about", "baths", "services", "journal", "news", "contacts", "sitemap"]


def published():
    return Publication.objects.filter(is_published=True, published_at__lte=timezone.localdate())
...
def bath_list(request):
    return render(request, "baths.html", {
...
def services(request):
    return render(request, "services.html", {
        "services": Service.objects.all(), "baths": Bath.objects.all(),
...
def publication_list(request, is_news):
    ...
    for item in articles:
        item.related_articles = [other for other in articles if other.pk != item.pk and other.category == item.category][:2]
    return render(request, "publications.html", {
        "articles": articles, "is_news": is_news,
        "title": "Новости и предложения — Тихий пар" if is_news else "Журнал о банях и подготовке к посещению — Тихий пар",
        "description": "Новости комплекса, изменения в расписании и предложения для гостей." if is_news else "Что взять с собой, как выбрать помещение и рассчитать стоимость посещения. Практические статьи для гостей банного комплекса.",
    })
...
def contacts(request):
    ...
    if request.user.is_authenticated:
        initial = {"name": request.user.first_name or request.user.username, "email": request.user.email}
    ...
    form = InquiryForm(request.POST if request.method == "POST" else None, initial=initial)
    if request.method == "POST":
        if limited(request, "inquiry", 5):
            form.add_error(None, "Слишком много обращений. Попробуйте через пять минут.")
        elif form.is_valid():
            inquiry = form.save(commit=False)
            if request.user.is_authenticated:
                inquiry.user = request.user
            inquiry.save()
            messages.success(request, f"Обращение № {inquiry.pk} сохранено. Администратор подтвердит время отдельно.")
            if inquiry.user:
...
def account(request):
    ...
    if not request.user.is_authenticated and request.method == "POST":
        ...
        if mode == "register":
            ...
            if limited(request, "register", 5):
                register_form.add_error(None, "Попробуйте зарегистрироваться через пять минут.")
            ...
            if limited(request, "login", 10):
                login_form.add_error(None, "Слишком много попыток входа. Подождите пять минут.")
    ...
    current = None
    if request.user.is_authenticated:
        inquiries = Inquiry.objects.filter(user=request.user).select_related("bath").prefetch_related("replies__author")
        current = selected_inquiry(request, inquiries)
    return render(request, "account.html", {
        "register_form": register_form, "login_form": login_form, "mode": mode,
        "inquiries": inquiries, "current": current, "reply_form": MessageForm(),
        "title": "Личный кабинет — Тихий пар", "noindex": True,
    })
...
def reply(request, pk):
    inquiries = Inquiry.objects.all() if request.user.is_staff else Inquiry.objects.filter(user=request.user)
...
def staff(request):
    ...
    if request.method == "POST":
        ...
        if action == "publication":
            ...
            return redirect(reverse("staff") + "#users")
    inquiries = Inquiry.objects.select_related("bath", "user").prefetch_related("replies__author")
    return render(request, "staff.html", {
        "inquiries": inquiries, "current": selected_inquiry(request, inquiries),
        "reply_form": MessageForm(), "statuses": Inquiry.STATUSES,
        "publication_form": publication_form, "editing": editing,
        "publications": Publication.objects.all(), "users": User.objects.order_by("username"),
        "user_form": user_form, "title": "Управление сайтом — Тихий пар", "noindex": True,
    })
    ...
    if query:
        ...
        for item in Bath.objects.all():
            ...
            if all(word in text for word in words):
                results.append({"title": item.name, "text": item.description, "url": item.get_absolute_url(), "category": "Бани и сауны"})
        for item in Service.objects.all():
            text = (item.name + " " + item.description).casefold()
            if all(word in text for word in words):
                results.append({"title": item.name, "text": item.description, "url": item.get_absolute_url(), "category": "Услуги"})
        for item in published():
            text = (item.title + " " + item.summary + " " + item.body).casefold()
            if all(word in text for word in words):
                results.append({"title": item.title, "text": item.summary, "url": item.get_absolute_url(), "category": item.get_category_display()})
    return render(request, "search.html", {"query": query, "results": results, "title": "Поиск по сайту — Тихий пар", "noindex": True})


def sitemap(request):
    return render(request, "sitemap.html", {"title": "Карта сайта — Тихий пар", "description": "Все разделы сайта банного комплекса: помещения, услуги, журнал, новости, контакты и личный кабинет."})
...
def not_found(request, exception=None):
    return render(request, "error.html", {"code": 404, "heading": "Такой страницы нет", "explanation": "Проверьте адрес или вернитесь на главную.", "title": "Страница не найдена — Тихий пар", "noindex": True}, status=404)


def forbidden(request, exception):
    return render(request, "error.html", {"code": 403, "heading": "Нет доступа", "explanation": "Эта страница доступна администратору.", "title": "Нет доступа — Тихий пар", "noindex": True}, status=403)


def csrf_error(request, reason=""):
    return render(request, "error.html", {"code": 403, "heading": "Обновите страницу", "explanation": "Срок действия формы истёк. Откройте страницу заново и повторите отправку.", "title": "Повторите отправку — Тихий пар", "noindex": True}, status=403)
